...
function BuildListFea( )
    local entries = {};
    table.insert(entries, { Text=Locale.Lookup("LOC_MAP_EDITOR_FEATURES_UNCH"), Type=nil });
        ...
        --elseif type.ResourceClassType == "RESOURCECLASS_STRATEGIC" then
            ...
            if IsXP2() then
                if type.FeatureType == "FEATURE_FLOODPLAINS" or type.FeatureType == "FEATURE_FLOODPLAINS_GRASSLAND" or type.FeatureType == "FEATURE_FLOODPLAINS_PLAINS" then
                    ...
                    if tooltip ~= nil then
                        tooltip = tooltip .. "[NEWLINE][NEWLINE][COLOR_RED]" .. Locale.Lookup( "LOC_MAP_EDITOR_FEATURE_FLOODPLAINS_BUGGED" ) .. "[ENDCOLOR]";
                    else
                        tooltip = "[COLOR_RED]" .. Locale.Lookup( "LOC_MAP_EDITOR_FEATURE_FLOODPLAINS_BUGGED" ) .. "[ENDCOLOR]";
                    end
...
function BuildListRes( )
    local entries = {};
    table.insert(entries, { Text=Locale.Lookup("LOC_MAP_EDITOR_RESOURCES_UNCH"), Type=nil });
    ...
    for type in GameInfo.Resources() do
        ...
        if type.ResourceClassType == "RESOURCECLASS_ARTIFACT" then
            ttext = ttext .. "[NEWLINE][COLOR_RED]" .. Locale.Lookup("LOC_MAP_EDITOR_RESOURCE_BUGGED") .. "[ENDCOLOR]";
        elseif type.Frequency == 0 and type.SeaFrequency == 0 then
            -- minor civ luxury resource
            ttext = ttext .. "[NEWLINE][COLOR_RED]" .. Locale.Lookup("LOC_MAP_EDITOR_RESOURCE_MINOR_CIV") .. "[ENDCOLOR]";
        end
    ...
    for type in GameInfo.Improvements() do
        ...
        if tooltip ~= nil then 
            ...
            if type.ImprovementType == 'IMPROVEMENT_POLDER' or type.ImprovementType == 'IMPROVEMENT_MEKEWAP' then
                tooltip = tooltip .. "[NEWLINE][NEWLINE][COLOR_RED]" .. Locale.Lookup( "LOC_MAP_EDITOR_IMPROVEMENT_BUGGED" ) .. "[ENDCOLOR]";
            end
        end
...
function BuildListRivers( )
    local riverNum :number = RiverManager.GetNumRivers();
        ...
        local entries = {};
        --table.insert(entries, { Text=Locale.Lookup("LOC_MAP_EDITOR_CONTINENTS_UNCH"), Type=nil });
        table.insert(entries, { Text=Locale.Lookup("LOC_MAP_EDITOR_RIVER_NEW") .. " " .. riverNum, Type=riverNum });
